...
parser.add_argument('-file', default = "result_nntk+nngp.log", type = str, help = "Output File")
...
for idx, dataset in enumerate(sorted(os.listdir(datadir))):
    ...
    for repeat in range(4):
        ...
        x_val = K[val_fold][:, train_fold]
        x_test = K[test_fold][:, train_fold]
        acc = NNGP.GP(x_train,y[train_fold],x_test, y[test_fold],max_w,0,max_l,best_regul)
        avg_acc += 0.25 * acc
